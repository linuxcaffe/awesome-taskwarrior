...
run_debug_mode() {
    ...
    {
        echo "========================================================================"
        echo "make-awesome.sh --debug Session - $(date '+%Y-%m-%d %H:%M:%S')"
    ...
    echo "[$(date '+%H:%M:%S')] Found ${#python_files[@]} Python file(s): ${python_files[*]}" >> "$log_file"
    echo ""
    
    for pyfile in "${python_files[@]}"; do
        debug_name="debug.${pyfile}"
        
        echo "[$(date '+%H:%M:%S')] Processing: $pyfile" >> "$log_file"
        
        # Check if debug version already exists
        if [ -f "$debug_name" ]; then
            msg "Debug version exists: $debug_name"
            echo "[$(date '+%H:%M:%S')]   Debug version exists: $debug_name" >> "$log_file"
            
            # Check TW_DEBUG compatibility
            if check_tw_envar_compatibility "$debug_name"; then
                success "Already compatible with TW_DEBUG, skipping"
                echo "[$(date '+%H:%M:%S')]   Already compatible with TW_DEBUG, skipping" >> "$log_file"
                continue
            else
                warn "Not compatible with TW_DEBUG, regenerating..."
                echo "[$(date '+%H:%M:%S')]   Not compatible with TW_DEBUG, regenerating..." >> "$log_file"
            fi
        fi
        
        # Generate debug version (capture stderr to log file, strip color codes)
        echo "[$(date '+%H:%M:%S')]   Generating debug version..." >> "$log_file"
